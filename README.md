...
**At the garage** *(the original use case)*: ESP32 lives near the gate, powered by a wall outlet / power bank / solar, and is hidden in or near the entrance. When you're in BLE range, you tap the open button on the Android Auto screen and the ESP32 triggers the fob for you. The fob stays at the garage. This is the design the firmware is optimized for (low-power sleep, deep-sleep wake-on-BLE, etc.). It's also the only deployment that supports **multiple users** — anyone you share the password with (other family members, housemates, visiting friends) can install the app on their own phone and open the gate from their own vehicle, without you having to be there or hand anything over.
