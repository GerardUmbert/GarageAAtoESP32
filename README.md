...
**Multiple openers:** pair more than one device — any mix of BLE ESP32s and webhooks — instead of one replacing the last. Once 2+ devices are paired, every screen (phone, Android Auto, Wear tile, watch app, Quick Settings tile) gains a device picker so you choose which opener a tap fires. If you've set up a geofence per device, standing inside one (or more) of them auto-selects the matching device(s) everywhere, so multi-opener setups still feel like a single tap — see [docs/geofence-auto-open.md](docs/geofence-auto-open.md) for the full resolution logic.
